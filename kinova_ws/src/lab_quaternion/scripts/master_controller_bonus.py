...
def run_kinova():
    return subprocess.Popen(["bash", "-c", "export ROS_DOMAIN_ID=1 && python3 kinova_controller_bonus.py"])

def run_turtlebot():
    return subprocess.Popen(["bash", "-c", "export ROS_DOMAIN_ID=2 && python3 turtlebot_controller_bonus.py"])
